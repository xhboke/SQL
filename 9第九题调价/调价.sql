...
INSERT INTO #Temp 
	...
	SELECT S.ROOM_TYPE_ID,ROUND( CAST(SUM(入住天数) AS FLOAT)/60,3) 'RATE'  FROM
	(
		/** 换房的入住 */
		SELECT ROOM_TYPE.ROOM_TYPE_ID,SUM(DATEDIFF(DD,CHECKIN_START_TIME,CHANGE_TIME)) '入住天数' FROM CHECKIN 
	...
	union all
		/** 没有换房的入住 */
		SELECT ROOM_TYPE.ROOM_TYPE_ID,SUM(PLAN_TIME) '入住天数' FROM CHECKIN 
			LEFT JOIN ROOM_INFO ON CHECKIN.ROOM_ID = ROOM_INFO.ROOM_ID
			LEFT JOIN ROOM_TYPE ON ROOM_INFO.ROOM_TYPE_ID = ROOM_TYPE.ROOM_TYPE_ID
			LEFT JOIN CHANGE_ROOM ON CHANGE_ROOM.CHECKIN_ID = CHECKIN.CHECKIN_ID
			WHERE 
